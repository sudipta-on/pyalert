...
class Config:
    """
    Persisted pyalert configuration.

    The `webhook_url` is your own Google Apps Script Web App URL scoped
    to your Gmail account. PyAlert never requests, stores, or transmits
    a Gmail password.
    """
...
def load_config(path: Optional[Path] = None) -> Config:
    """Load config from disk, applying environment variable overrides on top."""
    ...
    # Environment variables take precedence for CI/HPC jobs
...
def run_wizard(existing: Optional[Config] = None) -> Config:
    """Interactive terminal wizard that produces a validated Config."""
    print("=" * 62)
 ...
 *
 * This script runs entirely inside YOUR Google account. It receives a
 * JSON payload over HTTPS POST from the pyalert Python client and uses
 * GmailApp (already authorized as *you*) to send a styled HTML email,
 * with optional base64 attachments. Your Gmail credentials never leave
 * Google's infrastructure and are never seen by pyalert or any third
 * party — pyalert only ever talks to this endpoint, which you own.
 *
 * DEPLOYMENT
 * 1. Go to https://script.google.com/ and create a New Project.
 * 2. Delete the boilerplate and paste in this entire file.
 * 3. Click Deploy -> New deployment -> select type "Web app".
 *      - Execute as: Me
 *      - Who has access: Anyone   (Secured via SHARED_SECRET)
 * 4. Click Deploy, authorize the requested Gmail scopes, and copy
 *    the "Web app URL" it gives you.
 * 5. Paste that URL into `pyalert-setup` when prompted for the
...
def _cmd_test(cfg: Config) -> int:
    """Send a test email using the current config (lazy import avoids cyclic loads)."""
    problems = cfg.validate()
    if problems:
...
def main(argv: Optional[list] = None) -> int:
    ...
    if args.generate_script:
        ...
        print(
            ...
            f"  2. Paste in the contents of {path} into Code.gs.\n"
            "  3. Deploy -> New deployment -> Select type: Web app\n"
            "       - Execute as: Me\n"
            "       - Who has access: Anyone  (Secured via the embedded shared secret)\n"
            "  4. Authorize Gmail access when prompted.\n"
            "  5. Copy the Web app URL and run `pyalert-setup` to save your setup.\n"
        )
        return 0
